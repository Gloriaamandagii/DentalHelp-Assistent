...
let sendWhatsAppMessage = async (phone, text) => {
  if (typeof sendWhatsappNotif === "function") {
    try {
      ...
      console.warn(
        "[chatbot.controller] sendWhatsappNotif failed:",
  ...
  console.warn(
    "[chatbot.controller] sendWhatsAppMessage: no underlying send function available"
  );
  return null;
};
...
const handleIncomingMessage = async (req, res) => {
  try {
    const number =
      req.body &&
      (req.body.sender || req.body.phone || req.body.from || req.body.pengirim);
    const message =
    ...
    const aiResult = await generateReplyFromGemini(
      message,
      formattedNumber,
      null,
      { autoSave: false }
    ...
    if (typeof aiResult === "string") reply = aiResult;
      ...
      reply = aiResult.reply || aiResult.deskripsi || null;
    ...
    return res
      .status(200)
      .json({ success: true, sent: false, message: "No reply from AI" });
  } catch (error) {
    console.error(
      "[WEBHOOK] Error:",
